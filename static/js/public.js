function getuser(){
  window.sessionStorage.token = getCookie("access_token").split("_")[0]
  var param = {token:getCookie("access_token").split("_")[0]}
  $.ajax({
    url:'//www.cq.gov.cn/public-ykbapi-oauth/oauth/userInfo',
    type:"post",
    data:param,
    async:true,
    timeout:3000,
    ...
    success:function(data){
      if(data.code == 0){   //用户已经登录
        $(".i-top-login").hide();
        $(".i-top-logined").show();
        $(".head-login").hide();
        $(".head-logined").show();
        if (setCookie) setCookie('loginStatus', '1')
          var userinfoUrl = 'http://zwykb.cq.gov.cn/cbl/grzx/'
          $("#loginusername").html(data.data.username).attr("href", userinfoUrl)
        if(data.data.username.length > 8){
          var _str = '<style>.l-top-info a{float:left;line-height:.3rem;}.l-top-info .l-zc-btn.userInfo{width:1.1rem;margin-left:.1rem!important}</style>'
          $(".l-top-info").append(_str)
        }
...
$(function(){
  ...
  $("#logout").click(function(){
    ...
    delCookie("mesbox")
    window.sessionStorage.token = ''
    window.location.href=logout +curURL;
  });
...
/*登录*/
var loginURL = "https://auth.cq.gov.cn:81/sso/login?utype=0&client_id=OCTVFKMB0&goto=";
var registerURL = "https://auth.cq.gov.cn:81/sso/register?utype=0&client_id=OCTVFKMB0&goto=";
var logout = "https://auth.cq.gov.cn:81/sso/logout?utype=0&client_id=OCTVFKMB0&goto=";
